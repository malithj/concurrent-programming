...
int main()
{
    ...
    for(seed=10; seed<11+iterations; seed++){
        ...
        while(thread_member_number<max_member_number || thread_insert_number<max_insert_number || thread_delete_number<max_delete_number){
        random_number = rand() % (MAX_NUMBER+1);
        operation = random_number % 3;   //Choose the operation randomly

        if(operation==0 && thread_member_number < max_member_number){
                thread_member_number++;
                Member(random_number, *head_node);
        }
        else if(operation==1 && thread_insert_number < max_insert_number){
                thread_insert_number++;
                Insert(random_number, head_node);
        }
        else if(operation==2 && thread_delete_number < max_delete_number){
